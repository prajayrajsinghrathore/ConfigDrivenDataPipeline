...
class TestDuckDBTypeCastStep:
    ...
    def test_invalid_dtype_raises_transformation_error(self):
        ...
        config = TypeCastStepConfig(
            type="type_cast", columns={"value": "not_a_real_dtype"}
        )
...
class TestDuckDBFormulaStep:
    ...
    def test_formula_with_functions(self):
        ...
        config = FormulaStepConfig(
            type="formula", columns={"upper_name": "upper(name)"}
        )
        result = _apply_step(DuckDBFormulaStep(), df, config)
    ...
    def test_formula_error_propagates(self):
        df = pl.DataFrame({"value": [1, 2, 3]})
        config = FormulaStepConfig(
            type="formula", columns={"bad_col": "nonexistent_column * 2"}
        )
...
class TestDuckDBFilterStep:
    ...
    def test_chained_filters_via_pipeline(self):
        ...
        )  # noqa: F841 (used via duckdb frame introspection)
        rel = duckdb.sql("SELECT * FROM df")
        data = DuckDBData(rel)
        ...
        plan = PipelinePlanner.create_plan(
            config, SourceSpec(path="dummy"), DestinationSpec(path="dummy")
        )
        ctx = _dummy_context()
...
class TestDuckDBAggregationStep:
    def test_sum_aggregation(self):
        ...
        config = AggregationStepConfig(
            type="aggregation", aggregations={"sum": ["value"]}
        )
    ...
    def test_skip_nonexistent_column(self):
        df = pl.DataFrame({"value": [10, 20, 30]})
        config = AggregationStepConfig(
            type="aggregation", aggregations={"sum": ["value", "nonexistent"]}
        )
        result = _apply_step(DuckDBAggregationStep(), df, config)
...
class TestDuckDBSnowflakeLookupStep:
    ...
    @patch(
        "rlam_airflow_framework.engine.transformers.snowflake_lookup.SnowflakeLookupProvider.get_lookup_dataset"
    )
    ...
    def test_no_matches_left_join_preserves_rows(self, mock_get):
        ...
        df = pl.DataFrame(
            {"instrument_id": ["INS001", "INS002"], "quantity": [100, 200]}
        )
        result = _apply_step(
            DuckDBSnowflakeLookupStep(), df, self._config(select_columns=["ISIN"])
        )
        assert len(result) == 2
