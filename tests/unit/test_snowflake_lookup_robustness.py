...
def mock_snowflake_hook():
    with patch("rlam_airflow_framework.engine.transformers.snowflake_lookup.SnowflakeHook") as MockHook:
...
class TestSnowflakeLookupRobustness:
    ...
    def test_no_pandas_imported_in_execution_path(self, mock_snowflake_hook):
        """
        ...
        def trap_pandas_import(name, globals=None, locals=None, fromlist=(), level=0):
            if name == "pandas":
                raise ImportError("Pandas was imported dynamically in the execution path!")
            return original_import(name, globals, locals, fromlist, level)
            
        with patch("builtins.__import__", side_effect=trap_pandas_import):
            result = step.transform(data, _config(), _dummy_context())
    ...
    def test_multiple_arrow_batches(self, mock_snowflake_hook):
        ...
        batches = [
            pa.Table.from_pydict({"INSTRUMENT_ID": ["INS001"], "ISIN": ["US1"], "INSTRUMENT_NAME": ["N1"]}),
            pa.Table.from_pydict({"INSTRUMENT_ID": ["INS002"], "ISIN": ["US2"], "INSTRUMENT_NAME": ["N2"]}),
            pa.Table.from_pydict({"INSTRUMENT_ID": ["INS003"], "ISIN": ["US3"], "INSTRUMENT_NAME": ["N3"]})
        ]
    ...
    def test_timestamp_precision_argument(self, mock_snowflake_hook):
        ...
        mock_snowflake_hook.fetch_arrow_batches.assert_called_with(force_microsecond_precision=True)
    ...
    def test_schema_mismatch_between_batches_throws(self, mock_snowflake_hook):
        ...
        batches = [
            pa.Table.from_pydict({"INSTRUMENT_ID": ["INS001"], "ISIN": ["US1"], "INSTRUMENT_NAME": ["N1"]}),
            pa.Table.from_pydict({"INSTRUMENT_ID": ["INS002"], "ISIN": [2.0], "INSTRUMENT_NAME": [True]}) # Type mismatch
        ]
        mock_snowflake_hook.fetch_arrow_batches.return_value = iter(batches)
    ...
    def test_scratch_lifetime_after_multiple_transformations(self, mock_snowflake_hook):
        ...
        from rlam_airflow_framework.engine.transformers.filter import DuckDBFilterStep, FilterStepConfig
        step2 = DuckDBFilterStep()
        data2 = step2.transform(data1, FilterStepConfig(type="filter", condition="true"), _dummy_context())
    ...
    def test_disk_full_or_parquet_write_failure(self, mock_snowflake_hook):
        ...
        with patch("pyarrow.parquet.write_table", side_effect=OSError("No space left on device")):
            with pytest.raises(ValueError, match="Snowflake lookup failed: No space left on device"):
                step.transform(DuckDBData(duckdb.sql("SELECT * FROM df")), _config(), _dummy_context())
