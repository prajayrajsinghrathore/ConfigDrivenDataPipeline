...
class TestSaveDataFrame:
    ...
    def test_save_dataframe_with_index(self, tmp_path):
        ...
        assert isinstance(loaded_df.index, pd.RangeIndex)
...
class TestRoundTripDataFrame:
    ...
    def test_multiple_saves_different_tasks(self, tmp_path):
        ...
        with patch("rlam_airflow_framework.taskflow_tasks.TEMP_DATA_DIR", tmp_path):
            ...
            # Load and verify
            pd.testing.assert_frame_equal(df1, _load_dataframe(path1))
            pd.testing.assert_frame_equal(df2, _load_dataframe(path2))
            pd.testing.assert_frame_equal(df3, _load_dataframe(path3))
...
class TestEdgeCases:
    ...
    def test_save_dataframe_with_categorical_dtype(self, tmp_path):
        ...
        assert loaded_df["category"].dtype.name == "category"
        pd.testing.assert_frame_equal(df, loaded_df)
