...
| 3C.1 Helm/AKS upgrade (chart, apiServer, images) | 🟨 | | 2026-07-25: REAL k8s sanity deploy on Docker Desktop (official chart 1.22.0 + our 3.3.0 image) — all 8 pods 1/1, API /version=3.3.0. apiServer schema VERIFIED. Found+fixed chart-native entrypoint (/entrypoint) — compose+k8s both green. See helm/LOCAL_K8S_DEPLOY.md. Still needs: 3.3.0-supporting chart when published, /clean-logs decision, ACR image+digest, dev-AKS deploy with prod storage/istio/identity |
