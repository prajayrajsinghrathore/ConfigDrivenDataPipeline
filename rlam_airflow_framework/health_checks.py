...
def check_kafka_health(
    ...
    log.info(
        "Starting Kafka health check", trace_id=trace_id, bootstrap_servers=servers
    )
...
if AIRFLOW_AVAILABLE and task is not None:
    ...
    @task.sensor(
        poke_interval=DEFAULT_POKE_INTERVAL, timeout=DEFAULT_TIMEOUT, mode="reschedule"
    )
    def wait_for_kafka_health(  # pyright: ignore[reportRedeclaration]
        bootstrap_servers: Optional[str] = None,
    ) -> PokeReturnValue:  # pyright: ignore[reportRedeclaration]
        """
