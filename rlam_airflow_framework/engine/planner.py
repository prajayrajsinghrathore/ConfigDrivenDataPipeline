...
class TransformerResolver:
    ...
    @staticmethod
    def resolve(
        step_config: TransformationConfig, backend: DataBackend
    ) -> TransformationStep:
        # Import dynamically to avoid circular imports during setup
        from rlam_airflow_framework.engine.transformers.formula import (
            DuckDBFormulaStep,
            PolarsFormulaStep,
        )
        from rlam_airflow_framework.engine.transformers.filter import DuckDBFilterStep
        from rlam_airflow_framework.engine.transformers.type_cast import (
            DuckDBTypeCastStep,
        )
        from rlam_airflow_framework.engine.transformers.aggregation import (
            DuckDBAggregationStep,
        )
    ...
    def create_plan(
        ...
        for step_config in config.transformations:
            ...
            resolved_steps.append(
                (target_backend, PlannedStep(transformer, step_config))
            )
