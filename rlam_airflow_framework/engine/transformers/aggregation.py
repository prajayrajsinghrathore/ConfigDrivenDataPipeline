...
class DuckDBAggregationStep(TransformationStep[AggregationStepConfig]):
    ...
    def output_backend(self, input_backend: DataBackend, config: AggregationStepConfig) -> DataBackend:
        return DataBackend.DUCKDB
        
    def transform(self, data: ExecutionData, config: AggregationStepConfig, context: ExecutionContext) -> ExecutionData:
