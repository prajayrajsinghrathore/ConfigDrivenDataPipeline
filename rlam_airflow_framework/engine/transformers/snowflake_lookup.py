...
class SnowflakeLookupProvider:
    ...
    @staticmethod
    def get_lookup_dataset(config: SnowflakeLookupConfig, unique_keys: list, output_dir: Path) -> Path:
        output_dir.mkdir(parents=True, exist_ok=True)
        
        if not unique_keys:
            # If no unique keys, create an empty dataset with the correct schema
            select_cols = list(config.join_on.values()) + config.select_columns
            schema = pa.schema([(col, pa.string()) for col in select_cols]) # Mock schema
        ...
        formatted_keys = [f"'{str(k).replace(chr(39), chr(39)+chr(39))}'" for k in unique_keys]
        in_clause = ", ".join(formatted_keys)
        ...
        try:
            with hook.get_conn() as connection:
                with connection.cursor() as cursor:
                    ...
                    part_number = 0
                    for table in cursor.fetch_arrow_batches(force_microsecond_precision=True):
...
class DuckDBSnowflakeLookupStep(TransformationStep[SnowflakeLookupConfig]):
    ...
    def output_backend(self, input_backend: DataBackend, config: SnowflakeLookupConfig) -> DataBackend:
        return DataBackend.DUCKDB
        
    def transform(self, data: ExecutionData, config: SnowflakeLookupConfig, context: ExecutionContext) -> ExecutionData:
        ...
        unique_keys_df = unique_keys_rel.pl() # Eager execution to get the list
        ...
        lookup_dataset_path = SnowflakeLookupProvider.get_lookup_dataset(config, unique_keys, temp_dir)
        ...
        joined_rel = rel.set_alias("lhs").join(
            lookup_rel.set_alias("rhs"), 
            f"lhs.{local_key} = rhs.{remote_key}", 
            "left"
        )
